...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		boolean flag = false;
		int count = 0;
		String syllables = new String("aeiouyAEIOUY");
		while(i<l) {
			if (syllables.indexOf(word.charAt(i)) != -1) {
				if (flag == false) {
					count += 1;
					flag = true;
					i += 1;
		...
		if (word.charAt(l-1) == 'e' && syllables.indexOf(word.charAt(l-2)) == -1 && count >=2) {
			count -= 1;
		}
	    return count;
	}
	...
	public double getFleschScore()
	{
		...
		double flesch = 206.835 - 1.015*this.getNumWords()/this.getNumSentences() - 84.6*this.getNumSyllables()/this.getNumWords();
	    return flesch;
	}
